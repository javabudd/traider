"""MCP server exposing read-only SEC EDGAR lookups to an AI CLI.
...
logger = logging.getLogger("sec_edgar_connector")
...
def search_companies(query: str, limit: int = 20) -> dict[str, Any]:
    ...
    CIK; every other tool in this server accepts a ticker or CIK via
    ``ticker_or_cik``.
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="sec-edgar-connector")
    parser.add_argument(
        "--transport",
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            "SEC_EDGAR_CONNECTOR_LOG", "logs/server.log",
        ),
    ...
    logger.info(
        "sec-edgar-connector starting transport=%s host=%s port=%s log=%s",
