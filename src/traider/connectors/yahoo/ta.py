"""TA-Lib indicator runner over OHLCV candles.
...
def _candles_to_inputs(candles: list[dict[str, Any]]) -> dict[str, np.ndarray]:
    """Turn the connector's candle list into the dict TA-Lib's abstract API wants."""
    if not candles:
...
def run_indicators(
    ...
    """Compute TA-Lib indicators on an OHLCV candle list.
